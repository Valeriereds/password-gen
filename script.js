...
function generatePassword() {
  var howMany = window.prompt(
    "How many characters do you want in your password?"
  );
  // console.log(howMany);
  // prompt for how many characters
  // call the function genpass() within function
  if (!howMany) {
    generatePassword()
    return;
  }
  ...
  if (!doSpec && !doNumb && !doUpper && !doLower) {
    window.alert(
      "You must choose at least one type of character to create a password. Please try again."
    );
    generatePassword();
  }
  var newNum = [];
